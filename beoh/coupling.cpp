...
extern  double  *fact;
static inline int parity(const int x){
  return( (((x)/2)%2 == 0) ? 1 : -1 );
}

static inline bool halfint(const int x){
...
{
  int i0 = max3(std::abs(j1-j9),std::abs(j2-j6),std::abs(j4-j8));
...
double triangle(const int a, const int b, const int c)
{
  ...
  if(c < std::abs(a-b) || c > (a+b)) return(0.0);
