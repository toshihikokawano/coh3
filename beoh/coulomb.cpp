...
static const int    MAX_ITERATION      = 100000;
static const double TINY_NUMBER        =  1e-64;
static const double HUGE_NUMBER        =  1e+64;
static const double VALUE_CUTOFF       =  1e-24;

static void  coulombBarnett (const int, const double, const double, std::complex<double> *, std::complex<double> *);
static void  coulombFRatio (const int, const double, const double, double *);
static void  coulombFrecur (const int, const double, const double, double *, std::complex<double> *, std::complex<double> *);
static std::complex<double> coulombWRatio (const int, const double, const double);
static double coulombPowerSeries (const double, const double);
static void  coulombClosed (const int, const double, const double, std::complex<double> *, std::complex<double> *);
...
/**********************************************************/
void coulomb_function(const int lmax, const double rho, const double eta, std::complex<double> *Coul0, std::complex<double> *Coul1)
{
  std::complex<double> *C0 = new std::complex<double>[MAX_L + 1]; // G + iF
  std::complex<double> *C1 = new std::complex<double>[MAX_L + 1]; // G'+ iF'
  ...
  for(int l=lmax+1 ; l<=lmax ; l++){
    Coul0[l] = std::complex<double>(0.0,0.0);
    Coul1[l] = std::complex<double>(0.0,0.0);
  }
...
void coulombBarnett(int lmax, double rho, double eta, std::complex<double> *c0, std::complex<double> *c1)
{
  double fw[3];
  ...
  std::complex<double> w = coulombWRatio(0, eta, rho);
  ...
  if(f0 == 0.0){
    for(int l=0 ; l<=lmax ; l++){
      c0[l] = std::complex<double>( HUGE_NUMBER,TINY_NUMBER);
      c1[l] = std::complex<double>(-HUGE_NUMBER,TINY_NUMBER);
    }
  }
  else{
    ...
    c0[0] = std::complex<double>(g0,f0);
    c1[0] = std::complex<double>(g1,f1);
    ...
    for(int l=1 ; l<=lmax ; l++){
      ...
      c0[l] = std::complex<double>(g0,f0);
      c1[l] = std::complex<double>(g1,f1);
    }
  }
...
void coulombFRatio(const int lambda, const double eta, const double rho, double *fw)
{
  ...
  do{
    ...
    if(fabs(f0 / f1 - 1.0) < CRITERIA_ITERATION){
      /*** save sign */
      if(d1 < 0.0) neg = true;
      conv = true;
      break;
...
/**********************************************************/
void coulombFrecur(const int lambda, const double eta, const double rho, double *fw, std::complex<double> *c0, std::complex<double> *c1)
{
  ...
  for(int l = lambda ; l>=1 ; l--){
    ...
    c0[l] = std::complex<double>(r1,f1);
    c1[l] = std::complex<double>(s1,0.0);
...
/**********************************************************/
std::complex<double> coulombWRatio(const int lambda, const double eta, const double rho)
{
  std::complex<double>x0(-lambda,eta);     // (-lambda, eta)
  std::complex<double>x1(lambda+1.0,eta);  // (lmabda+1,eta)
  std::complex<double>x2(2*(rho-eta),2.0); // (2(rho-eta), 2)

  std::complex<double>d0 = 1.0/x2;
  std::complex<double>h0 = x0*x1/x2;
  std::complex<double>z0 = h0;

  bool conv = false;
  int itr = 0;
  do{
    x0 += std::complex<double>(1.0,0.0);
    x1 += std::complex<double>(1.0,0.0);
    x2 += std::complex<double>(0.0,2.0);

    std::complex<double>d1 = 1.0 / (x2 + x0*x1*d0);
    std::complex<double>h1 = (x2*d1 - 1.0)*h0;
    std::complex<double>z1 = z0 + h1;

    double c = fabs(norm(z0)/norm(z1) - 1.0);
  ...
  z0 = std::complex<double>(0.0,1.0-eta/rho) + z0 * std::complex<double>(0.0,1.0/rho);
...
/**********************************************************/
void coulombClosed(const int lmax, const double rhox, const double eta, std::complex<double> *C0, std::complex<double> *C1)
{
  double p1, p2, p3;
  double rho = fabs(rhox);
  ...
  if(eta == 0.0){
    ...
    p1 = 1.0/rho;
    p2 = p1+1.0/(rho*rho);
    C0[0] = std::complex<double>(p1,0.0);
    C0[1] = std::complex<double>(p2,0.0);

    for(int l=1 ; l<lmax ; l++){
      p3 = (2.0*l+1.0)/rho*p2 + p1;
      C0[l+1] = std::complex<double>(p3,0.0);
      p1 = p2;  p2 = p3;
    }
    ...
    for(int l=1 ; l<=lmax ; l++){
      ...
      C1[l] = std::complex<double>(wr,0.0);
    }
    ...
    for(int l=lp ; l>=0 ; l--){
      ...
      x  = -(a*a - 1.0)/(b + x) + b;
      if(l <= lmax) C1[l] = std::complex<double>(x, 0.0);
    }

    for(int l=0 ; l<=lmax ; l++){
      double g1 = C1[l].real();
      double d  = 1.0/sqrt(fabs(g1 - f1));
      C0[l] = std::complex<double>(d,d);
      C1[l] = std::complex<double>(g1*d,f1*d);
